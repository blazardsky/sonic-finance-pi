...
func migrateStep(db *sql.DB, v int) error {
	...
	case 16:
		err = migrateCategoryColor(tx)
	case 17:
		err = migrateHoldingQuantityAndPrice(tx)
	case 18:
		err = migrateHoldingQuantityAdjustment(tx)
	case 19:
		err = migrateIncomeInvoiceAndExtra(tx)
	case 20:
		err = migrateHoldingCostAdjustment(tx)
	case 21:
		err = migrateSpendingIntent(tx)
	case 22:
		err = migratePlannedPurchase(tx)
	default:
